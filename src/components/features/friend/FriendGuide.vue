...
const siteConfig = {
  ...
  avatar: 'https://image.talen.top/1716712855.png',
  screenshot: 'https://image.talen.top/web/screenshot.png',
...
// 站点信息模板 - 使用配置对象动态生成
const siteTemplates = {
  yaml: `- name: ${siteConfig.name}
  link: ${siteConfig.link}
  avatar: ${siteConfig.avatar}
  descr: ${siteConfig.description}`,
  json: `{
  ...
  html: `<a href="${siteConfig.link}">${siteConfig.name}</a>`
}
...
// 复制站点信息
const copySiteInfo = (type: 'yaml' | 'json' | 'html') => copyToClipboard(siteTemplates[type])
  ...
  <details class="fold">
    ...
    <div class="fold-content">
      ...
      <p>
        ...
        <a href="mailto:talen2004@163.com" class="highlight">talen2004@163.com</a>。
      </p>
    </div>
  </details>
  ...
  <div class="tabs">
    ...
    <div class="tab-content">
      <div v-show="activeTab === 'yaml'" class="code-block">
        ...
        <pre><code>- name: {{ siteConfig.name }}
  link: {{ siteConfig.link }}
  avatar: {{ siteConfig.avatar }}
  descr: {{ siteConfig.description }}</code></pre>
      </div>
...
网站截图（可选）：</code></pre>
      ...
      <div v-show="activeApplyTab === 'form'" class="apply-format">
        <div class="form-apply-content">
          <div>
            ...
            <p>
              友链申请需要登录后才能提交，确保申请的真实性和可追踪性。
            </p>
          </div>
          <button class="submit-btn" @click="showApplyForm">
            表单提交
          </button>
        </div>
      </div>
  ...
  <!-- 友链申请表单弹窗 -->
  <BaseDialog v-model="showForm" title="友链申请" confirm-text="提交申请" :loading="formSubmitting" @confirm="submitApplyForm">
